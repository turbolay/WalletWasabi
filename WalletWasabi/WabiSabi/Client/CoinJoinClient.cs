...
public class CoinJoinClient
{
	...
	public async Task<CoinJoinResult> StartCoinJoinAsync(Func<Task<IEnumerable<SmartCoin>>> coinCandidatesFunc, CancellationToken cancellationToken)
	{
		...
		do
		{
			...
			var effectiveInputSum = coins
				.Sum(x => x.EffectiveValue(roundParameters.MiningFeeRate));
			if (smallestEffectiveDenom > effectiveInputSum)
			{
				throw new CoinJoinClientException(CoinjoinError.NoCoinsEligibleToMix, $"Skipping the round, because we don't have enough coins to register: the smallest possible denomination '{smallestEffectiveDenom}' BTC is larger than the effective input sum '{effectiveInputSum}' BTC.");
			}
	...
	private async Task<(ImmutableArray<AliceClient> aliceClientsThatSigned, IEnumerable<TxOut> OutputTxOuts, Transaction UnsignedCoinJoin)> ProceedWithRoundAsync(RoundState roundState, IEnumerable<SmartCoin> smartCoins, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			var effectiveInputSum = registeredAliceClientAndCircuits
				.Sum(x => x.AliceClient.EffectiveValue);
			if (smallestEffectiveDenom > effectiveInputSum)
			{
